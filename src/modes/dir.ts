...
import { detectCSSFrameworks } from '../extractors/css-frameworks';
import { runA11yAudit } from '../extractors/a11y-audit';
import { generateComponents } from '../writers/component-gen';
...
export async function runDirMode(projectDir: string, nameOverride?: string): Promise<DesignProfile> {
  ...
  const profile = normalize(projectName, frameworks, merged, components, libraries);

  // Step 8: Detect CSS frameworks
  profile.cssFrameworksDetected = detectCSSFrameworks(projectDir);

  // Step 9: A11y audit
  const allFilesContent = collectSourceContent(projectDir);
  profile.a11y = runA11yAudit(profile.colors, allFilesContent);

  // Step 10: Generate component code
  const hasReact = frameworks.some(f => f.id === 'react' || f.id === 'next');
  const hasVue = frameworks.some(f => f.id === 'vue' || f.id === 'nuxt');
  const targetFramework = hasVue ? 'vue' : hasReact ? 'react' : 'html';
  profile.generatedComponents = generateComponents(profile, targetFramework as any);
...
function collectSourceContent(projectDir: string): string {
  const parts: string[] = [];
  try {
    ...
    for (const dir of dirs) {
      ...
      if (fs.existsSync(fullDir)) {
        collectDirContent(fullDir, parts, 0);
      }
    }
    if (parts.length === 0) {
      collectDirContent(projectDir, parts, 0);
    }
  } catch { /* non-fatal */ }
  return parts.join('\n');
}

function collectDirContent(dir: string, parts: string[], depth: number): void {
  if (depth > 5) return;
  try {
    ...
    for (const entry of entries) {
      ...
      if (entry.isFile() && /\.(html?|tsx?|jsx?|vue|svelte|css|scss)$/i.test(entry.name)) {
        try {
          parts.push(fs.readFileSync(fullPath, 'utf-8'));
        } catch { /* skip unreadable */ }
      } else if (entry.isDirectory()) {
        collectDirContent(fullPath, parts, depth + 1);
      }
    }
  } catch { /* permission */ }
}
