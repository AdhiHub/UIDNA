...
export async function runUrlMode(url: string, nameOverride?: string, skillDir?: string): Promise<{ profile: DesignProfile; screenshotPath: string | null; cssColorCount: number; cssFontCount: number; computedColorCount: number; hadPlaywright: boolean }> {
  ...
  const htmlContent = httpResult.html || '';
  ...
  // Step 5: Detect CSS frameworks from HTML
  profile.cssFrameworksDetected = detectCSSFrameworks('', htmlContent);

  // Step 6: A11y audit on HTML
  profile.a11y = runA11yAudit(profile.colors, htmlContent);

  // Step 7: Generate component code
  profile.generatedComponents = generateComponents(profile, 'html');

  // Step 8: Capture screenshot (best-effort, non-blocking)
